    ...
    try{
      ...
      const balanceDetail = await Object.entries(balances).reduce(async (acc:any, coin:any) => {
        ...
        if(value?.available > 0){
          ...
          const marketValue = key == "THB" ? Number(value.available) * 1 : Number(value.available) * Number(marketPrice);
          balances.push({
            ...balances[key],
            symbol: key,
      ...
      const netWorth = balanceDetail.reduce((totalPrice:any, eachItem:any) => totalPrice+(Number(eachItem.marketValue)),0);
