...
interface CallbackType { (error: any, user: any, message: any): void }

const Authenticate = async (User: any, email: string, password: string, done: CallbackType) => {
  try{
    ...
    user.authenticate(password, (authError: any, authenticated: any) => {
      if (authError) return done(authError, false, { message: "Something went wrong, please try again." })
      if (!authenticated) return done(undefined, false, { message: "Invalid credentials, please try again." })
