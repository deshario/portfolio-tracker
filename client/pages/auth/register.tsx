  ...
  const [signup] = useMutation(SIGNUP, {
    onCompleted: ({ signup:{ _id } }) => {
      if(mongoose.Types.ObjectId.isValid(_id)){
        notification.success({ message: 'Register Success', description:'Please login to continue' })
        Router.push({ pathname: '/auth/login' })
      }
    },
    onError: (err) => {
      if(err?.message.includes('duplicate')){
        notification.error({ message: 'Duplicate email detected', description: 'Please try new one' })
      }
    }
  })
