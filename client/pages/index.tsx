...
const Home: NextPage<IInitialProps> = ({ bptUser }) => {
  ...
  const { data, error } = useQuery(QUERY_BALANCE, {
  ...
  const PNL = ({ item }: any) => {
    ...
    return (
      <Tag
        color={profit.includes('-') ? 'red' : 'green'}
        style={{ marginLeft: 5 }}
      >
        {profitLabel}
      </Tag>
          ...
          <Tabs defaultActiveKey='portfolio'>
            <TabPane tab='Portfolio' key='portfolio'>
              <List
                ...
                renderItem={(item: any) => (
                  <List.Item
                    key={`${item.symbol}_${item.available}`}
                  ...
                  >
                    <List.Item.Meta
                      ...
                      title={
                        <a
                          href={getCoinInfo(item.symbol, true)}
                          target='_blank'
                        >
                          {item.symbol}
                        </a>
            ...
            <TabPane tab='Top Performance' key='performance'>
              <List
                dataSource={balances.listData
                  ...
                  .sort(
                    (a: any, b: any) =>
                      Number(b.profitPercent) - Number(a.profitPercent),
                  )}
                renderItem={(item: any) => (
                  <List.Item
                    key={`${item.symbol}_${item.available}`}
              ...
              options={{
                ...
                tooltip: {
                  formatter: function () {
                    ...
                    if (curElem?.key) {
                      const targetDep: any = balances.listData.find(
                        (e: any) => e.symbol == curElem?.key,
                      )
                      if (targetDep) {
                        const {
                          symbol,
                        ...
                        return `
                            ...
                            ${isPositive ? 'Profit' : 'Loss'}: ${Number(
                              profitPercent,
                            ).toFixed(2)}% | ${thbCurrency(
                              Number(marketValue) - Number(totalBought),
                            )}<br/>
                ...
                series: [
                  {
                    data: balances.listData.map((eachItem: any) => {
                      return {
                        ...
                        y: Number(
                          Number(eachItem.marketValue) -
                            Number(eachItem.totalBought),
                        ),
                      }
                    }),
                  },
