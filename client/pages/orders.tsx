...
const Orders: NextPage<IInitialProps> = () => {
  ...
  const { data, error } = useQuery(QUERY_DEPOSIT_SYMBOLS, { fetchPolicy: "network-only" })

  const [ getOrderHistory ] = useLazyQuery(QUERY_ORDER, {
    fetchPolicy: "network-only",
    onCompleted: ({ getOrderHistory : { data } }) => {

      const total = data.reduce((total:any,each:any) => {
                ...
                <TabPane tab={sym.replace('THB_','')} key={sym}>
                  {
                    isLoading ? <Loader/> :  <Table
                      ...
                      columns={[
                        ...
                        { 
                          title : (<Tooltip placement="topLeft" title={ovData?.totalAmount ? `Total : ${ovData?.totalAmount}` : ''}>Amount</Tooltip>),
                          dataIndex: 'amount', key: 'amount',
                        },
                        { title: 'Type', dataIndex: 'type', key: 'type', width:'10px', align:'center' },
                        { 
                          title : (<Tooltip placement="topLeft" title={ovData?.avgPrice ? `Average : ${thbCurrency(ovData?.avgPrice)}` : ''}>Rate</Tooltip>),
                          dataIndex: 'rate', key: 'rate', align:'center',
                        },
                        { 
                          title : (<Tooltip placement="topLeft" title={ovData?.totalFees ? `Total Fees : ${thbCurrency(ovData?.totalFees)}` : ''}>Fees</Tooltip>),
                          dataIndex: 'fees', key: 'fees', align:'center',
                        },
                        { title: 'Profit', dataIndex: 'profit', key: 'profit', align:'center' },
