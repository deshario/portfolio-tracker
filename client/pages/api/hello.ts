...
export default async function handler(req: NextApiRequest, res: NextApiResponse<TotalBalances>) {
  ...
  try{
    ...
    const availableBalances:Balances[] = Object.keys(balances).map(key => ({...balances[key], symbol:key})).filter(transaction => transaction.available > 0)
