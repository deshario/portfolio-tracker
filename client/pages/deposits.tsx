...
const Deposits: NextPage<IInitialProps> = () => {
  ...
  const { data, error } = useQuery(QUERY_ALL_DEPOSIT, {
  ...
  useEffect(() => {
    if (data && data.getAllDeposit) {
      ...
      const groupedFiat = fiat
        .reduce((grouped: any, eachItem: any) => {
        ...
        .map((eachMonth: any) => {
          const total = eachMonth.deposits.reduce(
            (total: Number, eachDep: any) => total + eachDep.amount,
            0,
          )
          return {
            name: eachMonth.date,
        ...
        .reverse()
      const totalFiatDeposit = groupedFiat.reduce(
        (total: Number, eachItem: any) => total + eachItem.y,
        0,
      )
            ...
            extra={
              <strong style={{ color: 'green' }}>
                <Tooltip
                  placement='topLeft'
                  title={
                    overViewData?.netWorth
                      ? `Real Value : ${thbCurrency(overViewData?.netWorth)}`
                      : ''
                  }
                >
              ...
              renderItem={(item: any) => (
                <List.Item key={item.time}>
                  <List.Item.Meta
                    ...
                    title={
                      <a
                        href={getCoinInfo(item.currency, true)}
                        target='_blank'
                      >
                        {item.currency}
                      </a>
